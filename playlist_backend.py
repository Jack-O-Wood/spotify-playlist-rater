...
# ----------------------------------------------------------
# 1) SPOTIFY CLIENT (copy your actual code from the notebook)
# ----------------------------------------------------------
# Example structure (replace with your real authentication code):
#
import spotipy
...
# ----------------------------------------------------------
# 3) YOUR DECISION THRESHOLD
# ----------------------------------------------------------
best_threshold_full = 0.871

# ----------------------------------------------------------
# 4) HELPER FUNCTIONS (paste EXACTLY as they appear)
# ----------------------------------------------------------
# Required functions:
#extract_playlist_id()
...
#rate_playlist()
#
# Paste the full function definitions below, no changes.

# --- paste load_playlist_tracks() here ---
...
def load_playlist_tracks(playlist_ref: str, sp_client, cache_buster=None) -> pd.DataFrame:
    ...
    while True:
        ...
        for item in batch:
            ...
            # ✅ NEW: album cover URL (take first image if present)
            ...
            items.append(
                {
                    ...
                    "album_image_url": album_image_url,   # ✅ new column
                }
            )
...
def fetch_audio_features(track_ids, sp_client) -> pd.DataFrame:
    ...
    NOTE (2025): Spotify's /v1/audio-features endpoint now returns 403
...
def follower_bucket(n_followers: float) -> str:
    ...
    Adjust thresholds if needed to match your original code.
    """
...
def genres_to_flags(genre_list):
    """
    Map a list of Spotify genres to your genre_* flags.
...
def enrich_playlist_for_model(df_playlist_meta, sp_client) -> pd.DataFrame:
    ...
    # Merge audio features only if we have them
    ...
    # --- NEW: ensure raw audio feature columns exist even if audio-features failed ---
    audio_cols = [
        "energy",
        "valence",
        "danceability",
...
def rate_playlist(
    ...
    # 5) Summary using your existing logic
